...
async def func_executor_coroutine(
    func: Any, loop: asyncio.AbstractEventLoop, executor: ThreadPoolExecutor
) -> Any:
...
class BoundedAsyncioExecutor(futures.Executor):
    ...
    def __init__(
        ...
        *,
        ...
        self._thread.start()
        self.semaphore = asyncio.Semaphore(max_workers, loop=self._loop)
    ...
    def submit(self, fn: Any, *args: Any, **kwargs: Any) -> futures.Future:
        ...
        if inspect.iscoroutinefunction(fn):
            ...
            coro = self._wrapped(fn, *args, **kwargs)
            ...
            coro = self._wrapped(
                func_executor_coroutine,
                func,
                loop=self._loop,
                executor=self._func_executor,
            )
            fu = asyncio.run_coroutine_threadsafe(coro, self._loop)
            return fu

    async def _wrapped(self, fn, *args, **kwargs):
