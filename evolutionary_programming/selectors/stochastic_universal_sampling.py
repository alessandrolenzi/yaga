...
class StochasticUniversalSampling(Selector[IndividualType]):
    def __call__(self, population: Sequence[Tuple[IndividualType, float]]) -> Iterable[IndividualType]:
        ...
        selected_points = [start + pointers_distance * i for i in range(self.selection_size)]
        total_seen_score = 0
        for individual, score in population:
            cumulative_mating_probability = score + total_seen_score
            while selected_points and total_seen_score <= selected_points[0] < cumulative_mating_probability:
                selected_points.pop()
                yield individual
